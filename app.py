...
if uploaded_file:
    ...
    available_fields = list(metadata_list[0].keys())
    ...
    pattern = st.text_input(
        "Filename Pattern",
        value=st.session_state.get("filename_pattern", default_pattern),
    )
    st.session_state.filename_pattern = pattern
    ...
    for col, field in zip(cols, available_fields):
        if col.button(f"{{{field}}}"):
            ...
            pattern = st.session_state.filename_pattern
